...
impl ZKELLAGovernance {
    ...
    pub fn queue_vk_update(env: Env, circuit_id: u8, new_vk: Bytes) {
    ...
    pub fn execute_vk_update(env: Env, circuit_id: u8) -> Bytes {
        let admin: Address = env.storage().instance().get(&StorageKey::Admin).unwrap();
        admin.require_auth();
    ...
    pub fn cancel_vk_update(env: Env, circuit_id: u8) {
        let admin: Address = env.storage().instance().get(&StorageKey::Admin).unwrap();
        admin.require_auth();
        env.storage().instance().remove(&StorageKey::PendingVkUpdate(circuit_id));
    }
